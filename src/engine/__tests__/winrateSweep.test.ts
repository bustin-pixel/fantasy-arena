...
//
//   SWEEP=1 npm test -- winrateSweep                 # the whole curve
...
//   at cap + legendary gear ~50%. Walls (Sealed Vault, Eclipse Spire) sit a notch
//   lower on the boss floor; that's intended — the player replays/levels/gears up.
// ============================================================================
 ...
 *  notch above the dungeon's fodder level (having grinded the earlier ones) and
 *  gain roughly a level per floor as they descend, all capped at the Lv-10
 *  ceiling. Modeling the descent-leveling matters — a fixed level makes deep
 *  floors look far harder than they play. Tune target: ~65-75% F1-4, ~50% boss. */
function intendedLevel(monsterLevel: number, floor: number): number {
  return Math.min(10, monsterLevel + 1 + floor);
}
...
type GearTier = "none" | "rare2" | "epic1" | "leg1";
...
function gearFor(deck: string[], tier: GearTier): ItemLoadouts {
  ...
  const loadout = {
    ...
    ...(tier === "leg1" ? { trinket: makeItemKey("ember_charm", q, s) } : {}),
  };
...
/** Expected gear tier a player brings when they arrive at each dungeon (the plan's
 *  gear-expectation curve). Level expectation = the dungeon's monsterLevel. */
...
RUN("winrate sweep — progression curve", () => {
  ...
  it("prints the expected-arrival curve (descent-leveled per floor, gear per tier)", () => {
    ...
    for (const id of dungeons) {
      ...
      const gear = GEAR_BY_DUNGEON[id] ?? "none";
